...
public class RadioCompetition {
    ...
    private LecturaEscritura Registro;

    public RadioCompetition(LecturaEscritura lecturaEscritura) {
        this.Registro = lecturaEscritura;
    ...
    private void llenarCombo(JComboBox<Concurso> combo) throws IOException {
        var listaConcurso = Registro.todosLosConcursos();
        for (Concurso concurso : listaConcurso) {
            combo.addItem(concurso);
        }
    }
    ...
    private void saveInscription() {
        if (validations()) {
            ...
            try {
                Registro.guardarInscripcion(participante,concursoSeleccionado);
            } catch (IOException e) {
                JOptionPane.showMessageDialog(this.contentPane, "Error al guardar la inscripción: " + e.getMessage());
